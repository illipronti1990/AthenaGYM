...
export default async function AgendaHubPage() {
  ...
  return <Page {...pageQualityAttrs()}><PageHeader title="Agenda" description="Aulas, reservas, salas, presença e agenda dos professores." /><PageFilters>{AGENDA_LINKS.slice(1).map(([label, href]) => <Link key={href} href={href} className="athena-chip-nav">{label}</Link>)}</PageFilters><PageContent><AgendaDashboard accessToken={token} /></PageContent></Page>;
}
