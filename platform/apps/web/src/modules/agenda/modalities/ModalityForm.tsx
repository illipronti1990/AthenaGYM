...
import type { Modality } from '@athena/shared';
...
export function ModalityForm({ accessToken }: { accessToken: string }) {
  ...
  return <div className="space-y-5">{error ? <p className="text-sm text-red-600">{error}</p> : null}<form onSubmit={submit} className="flex flex-wrap items-end gap-3"><label className="text-sm">Nome<input className="athena-input mt-1 block" value={name} onChange={(e) => setName(e.target.value)} required /></label><label className="text-sm">Cor<input className="mt-1 block h-10 w-16" type="color" value={color} onChange={(e) => setColor(e.target.value)} /></label><button className="athena-btn athena-btn-primary">Adicionar</button></form><div className="grid gap-2 sm:grid-cols-2">{items.map((item) => <div key={item.id} className="rounded border border-[var(--border)] p-3"><span className="mr-2 inline-block h-3 w-3 rounded-full" style={{ background: item.color }} />{item.name}<span className="float-right text-xs text-[var(--muted)]">{item.defaultCapacity} vagas</span></div>)}</div></div>;
}
