      ...
      try {
        ...
        if (
          ...
          window.dispatchEvent(
            new CustomEvent('movvo-agenda-changed', { detail: { action } }),
          );
        }
      } catch (e) {
  ...
  return (
    ...
    >
      {!compact && (
        <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <p className="movvo-title text-base">Movvo AI</p>
      ...
      <div className="flex flex-wrap gap-2 border-b border-[var(--border)] px-3 py-2">
        {SUGGESTIONS[persona].slice(0, compact ? 3 : 4).map((s) => (
          <button
            key={s}
            type="button"
            className="movvo-chip-nav text-left text-xs"
            disabled={pending}
            onClick={() => send(s)}
          >
            {s}
      ...
      <div className="flex-1 space-y-3 overflow-y-auto px-3 py-3">
        ...
        {pending && (
          <div className="text-xs text-[var(--muted)]">Movvo está digitando…</div>
        )}
        <div ref={bottomRef} />
      </div>
      ...
      >
        <input
          ...
          data-testid="movvo-chat-input"
        />
        <Button type="submit" disabled={pending || !input.trim()} data-testid="movvo-chat-send">
          Enviar
        </Button>
      </form>
    </div>
